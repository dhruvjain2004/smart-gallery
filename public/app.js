...
// Image sources for the gallery - using reliable placeholder images with local fallbacks
const imageSources = [
...
// Local fallback image if remote images fail to load
const localPlaceholderImage = 'images/placeholder.svg';
 ...
 */
/**
 * Show error message with retry button when image loading fails
 */
function showImageError(container, src, index) {
...
function loadAndProcessImage(container, src, index) {
    ...
    img.onerror = function() {
        ...
        fallbackImg.onerror = function() {
            ...
            showImageError(container, src, index);
        };
        
        // Use our local placeholder image first, then fall back to a reliable CDN if needed
        fallbackImg.src = localPlaceholderImage;
        
        // If the local placeholder fails (which shouldn't happen), use a reliable CDN as last resort
        fallbackImg.onerror = function() {
            console.warn('Local placeholder image failed to load, using CDN fallback');
            fallbackImg.onerror = function() {
                // If even the CDN fallback fails, show error message with retry button
                showImageError(container, src, index);
            };
